...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    decalScale = float4(0.5,0.5,0,0);
        ...
        #endif
        uv = decalUV(float(0), float4(0.5,0.53,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
        decalColor = POI2D_SAMPLER_PAN(_DecalTexture, _MainTex, uv, float4(0,0,0,0)) * float4(0,0,0,1);
    #else
        uv = decalUV(float(0), float4(0.5,0.53,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
    ...
    albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha + audioLinkDecalAlpha0));
    ...
    decalEmission += decalColor.rgb * decalColor.a * max(float(0) + audioLinkDecalEmission0, 0);
        ...
        decalRotation = 0;
            ...
            #endif
            uv = decalUV(float(0), float4(0.5,1,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = POI2D_SAMPLER_PAN(_DecalTexture1, _MainTex, uv, float4(0,8,0,0)) * float4(0,0,0,1);
        #else
            uv = decalUV(float(0), float4(0.5,1,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = float4(0,0,0,1);
        #endif
        decalColor.rgb = decalHueShift(float(0), decalColor.rgb, float(0), float(0));
        decalColor.a *= applyTilingClipping(float(1), uv) * decalMask.g;
        ...
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(float(0.261) + audioLinkDecalAlpha1));
        ...
        decalRotation = 0;
            ...
            #endif
            uv = decalUV(float(0), float4(0.51,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = POI2D_SAMPLER_PAN(_DecalTexture2, _MainTex, uv, float4(0,0,0,0)) * float4(0,0,0,1);
        #else
            uv = decalUV(float(0), float4(0.51,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
            decalColor = float4(0,0,0,1);
        #endif
